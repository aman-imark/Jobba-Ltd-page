...
class ZipArchiver implements Service, ArchiverInterface
{
    ...
    public function uncompress(string $source, string $destination, Storage $storage)
    {
        ...
        $contents = array_map(function ($item) use($destination, $storage) {

            if (!empty($item['dirname'])) {
                while ($storage->fileExists($destination . $item['dirname'])) {
                    $item['dirname'] = $storage->upcountName($item['dirname']);
                }
            }
