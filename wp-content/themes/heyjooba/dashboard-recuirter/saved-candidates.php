<?php 
/*
Template Name:Saved Candidates
*/
include(TEMPLATEPATH.'/header-recruiter.php'); 
global $wpdb;
$id = get_current_user_id();
$rejected_candidates = $wpdb->get_results("SELECT * FROM `save_candidate` WHERE `rec_id` = $id AND `status` LIKE 'rejected'");

$act_candidate_data = $wpdb->get_results("SELECT * FROM `save_candidate` WHERE `rec_id` = $id AND `status` LIKE 'active'");
            ...
            <div class="cmn-jobs cmn-mg-btm bg-white-shadow">
                <div class="your-candidate cmn-job-table-wrap">
                    <table class="csm-job-table">
                        ...
                        <tbody>
                            <?php
                            foreach($saved_candidates as $candidate_dt){
