<?php 
/*
Template Name:Your Candidate
*/
include(TEMPLATEPATH.'/header-recruiter.php'); 
global $wpdb;
$id = get_current_user_id();

$candidate_data = $wpdb->get_results("SELECT * FROM `save_candidate` WHERE `rec_id` = $id AND `shared_status` = 0 AND `status` NOT IN ('save','rejected','offer-accepted','pending','contract-signed')");

$reject_candidate_data = $wpdb->get_results("SELECT * FROM `save_candidate` WHERE `rec_id` = $id AND  `shared_status` = 0 AND `status` LIKE 'rejected'");
                        ...
                        <tbody>
                            ...
                            $candidate_detail_data = unserialize($candi_dt);
                                if($candidate_status == 'active'){
                                   ...
                                   $status = 'Pending for Interview Time Confirmation'; 
                                }else if($candidate_status == 'interview-rearrange'){
                                   $status = 'Interview Rearrange';  
                                }else if($candidate_status == 'interview-booked' && $stage == 'stage1'){
                                   $status = 'Interview Booked for Stage 1';   
                                }else if($candidate_status == 'interview-booked' && $stage == 'stage2'){
                                   $status = 'Interview Booked for Stage 2';   
                                }else if($candidate_status == 'interview-booked' && $stage == 'stage3'){
                                   $status = 'Interview Booked for Stage 3';   
                                }else if($candidate_status == 'interview-booked' && $stage== 'stage4'){
                                   $status = 'Interview Booked for Stage 4';   
                                }else if($candidate_status == 'offer-pending'){
                                   $status = 'Offer Pending for confirmation';   
                                }else if($candidate_status == 'offer-accepted'){
                                   $status = 'Contract Pending';   
                                }else if($candidate_status == 'offer-rejected'){
                                   $status = 'Offer Rejected';   
                                }else if($candidate_status == 'interview-ongoing' && $stage == 'stage1'){
