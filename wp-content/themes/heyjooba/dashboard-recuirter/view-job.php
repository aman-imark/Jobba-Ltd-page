<?php
/*
Template Name:Recruiter View Job
*/
?>
<!-- <!doctype html> -->
<html lang="en">
...
/*****************None option***********************/
if($_GET['timings'] == 'none'){
  $row_id = $_GET['rid'];
  $job_id = $_GET['jb_id'];
 rearrange_interview($row_id,$job_id);
}
/*************For confirmation of timings(1st time)******************************/
    
if(!empty($_GET['timings']) && !empty($_GET['jb_id']) && $_GET['timings'] != 'none'){
     $selected_time = base64_decode($_GET['timings']);
    $row_id = $_GET['rid'];
    $job_id = $_GET['jb_id'];
    
    selected_time($selected_time,$row_id);
    
    $checkdata = $wpdb->get_results("SELECT * FROM interview_timing WHERE candidate_row_id=$row_id");
    if(empty($checkdata)){
        selected_time($selected_time,$row_id);
    }
...
/************************For confirmation of timings(updated)**********************************/
//if(!empty($_GET['edtimings']) && !empty($_GET['jb_id'])){
//    echo "hhh";
//    $selected_time = base64_decode($_GET['edtimings']);
//    $row_id = $_GET['rid'];
//    $job_id = $_GET['jb_id'];
//   update_selected_time($selected_time,$row_id);
//}
...
$job_cat = get_field('job_category',$job_id);
                                ...
                                <div class="submit-content">
                                    <div class="row">
                                        ...
                                        <div class="form-group d-flex col-md-6">
                                            ...
                                            <input type="text" class="form-control blank-field disabled"
                                            value="<?php echo date('d M,Y',strtotime($earlist_date)); ?>">
                                        </div>
                                        <div class="form-group d-flex col-md-6">
