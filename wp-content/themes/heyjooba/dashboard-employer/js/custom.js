(function (jQuery) {
  ...
  $('.wpcf7-form-control .wpcf7-list-item').on('click', function () {
    $(this).addClass('active').siblings('.wpcf7-list-item').removeClass('active');
  });
  ...
  $(document).on('click', '#ats_append .applicant-content > .applicant-details, #ats_append .applicant-content > figure', function (e) {
    e.preventDefault();
    $(this).parents('.applicant-content').find('.next-options').slideToggle();
  });
...
if(jQuery("#customrange2").length){
      document.getElementById("customrange2").oninput = function () {
    var value = (this.value - this.min) / (this.max - this.min) * 100
    this.style.background = 'linear-gradient(to right, #82CFD0 0%, #82CFD0 ' + value + '%, #d5d5d5 ' + value + '%, #d5d5d5 100%)'
  };
}
